...
function GradientBounds2D(r::AbstractVector{T}, p::Integer, f::AbstractVector{T}, φ::AbstractVector{T},
    ...
    Bb = (diff(Dp)' * P)[Block.(1:p+1), Block.(1:p)]
    B1 = ExtendableSparseMatrix(size(Bb)...)
    B1 .= Bb
    B1 = sparse(B1)
    B = kron(B1,B1)

    Gb = (Dp' * P)[Block.(1:p+1), Block.(1:p)]
    ...
    GradientBounds2D{T}(A, chol_A, p, Nh, B, E, D, M, G, P, Dp, plan_P, plan_tP, f, φ, φx, ψ, Ux, K1, K2)
end
...
#             ML3b[Nh*i-k,Nh*j-k] = ML3[i,j] #/ sqrt(cs[k+1])
#         end
#     end
